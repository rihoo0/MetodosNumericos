...
class Array2D
{
  ...
  public:
    ...
    explicit Array2D(const std::string &filename);
    bool write(const std::string &filename) const;

};

template <class T>
bool Array2D<T>::write(const std::string &filename) const
{
...
template <class T>
Array2D<T>::Array2D(const std::string &filename)
    : data_(), v_(), m_(0), n_(0)
{
    ...
    // Comprobar que todas las filas tienen el mismo número de columnas
    for (int i = 1; i < m_; i++)
    {
        if ((int)rows[i].size() != n_)
        {
            std::cerr << "Error: filas con distinto número de columnas\n";
            m_ = n_ = 0;
            return;
        }
    }

    // Reservar memoria contigua para los datos
    data_ = Array1D<T>(m_ * n_);

    // Crear el vector de punteros a filas
    v_ = Array1D<T*>(m_);
