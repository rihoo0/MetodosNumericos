...
int main()
{
  ...
  real x0=0.1;
  int NumeroIteraciones=mn_muller(f,x0,NiterMax,TOL);
  cout << "\nmuller : x0 = " << x0 << " , N. iteraciones = "
  << NumeroIteraciones << " , Gamma(x0) = " << Gamma(x0) << "\n";

  /// CALCULO EN LOS ÚLTIMOS DÍAS TRAS LA INFECCIÓN
  real x1=20.;
  NumeroIteraciones = mn_muller(f,x1,NiterMax,TOL);
  cout << "\nmuller : x1 = " << x1 << " , N. iteraciones = " <<
  NumeroIteraciones << " , Gamma(x1) = " << Gamma(x1) << "\n";

  /// CALCULO DE DONDE ALCANZA EL MÁXIMO LA DISTRIBUCIÓN GAMMA
  real xmax=2;
  NumeroIteraciones=mn_muller(Gammap,xmax,NiterMax,TOL);
  cout << "\nmuller : xmax = " << xmax << " , N. iteraciones = " <<
