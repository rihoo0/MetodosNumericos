...
class Array2D
{
  ...
  void write_image2D(char name[300]);
  T percentil(double p);
	...
	Array2D<T> transpose();
	Array2D<T> normalizar(T min_value,T max_value);
...
template <class T>
Array2D<T> Array2D<T>::normalizar(T min_value,T max_value){
  TNT::Array2D<T> tran((*this).dim1(), (*this).dim2(), (T) 0);
  T minI=(*this)[0][0],maxI=(*this)[0][0];
	for(int r=0; r<(*this).dim1(); ++r){
		for(int c=0; c<(*this).dim2(); ++c){
			if((*this)[r][c]>maxI) maxI=(*this)[r][c];
			else if((*this)[r][c]<minI) minI=(*this)[r][c];
		}
	}
	if(minI==maxI) return(tran);
	double s=(double) (max_value-min_value)/(maxI-minI);

	for(int r=0; r<(*this).dim1(); ++r)
		for(int c=0; c<(*this).dim2(); ++c)
			tran[r][c] = (T) ((double) min_value + s*((*this)[r][c]-minI));
...
template <class T>
void Array2D<T>::write_image2D(char name[300]){
...
template<class T>
T Array2D<T>::percentil(double p)
{
  vector<double> x((*this).dim1()*(*this).dim2());
