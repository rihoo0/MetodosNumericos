...
int main()
{
  ...
  /// CALCULAMOS CUANDO Gamma(x)=0.01 USANDO EL MÉTODO DE LA REGULA FALSI
  int NiterMax=1000;
  real TOL=1e-8;

  /// CALCULO EN LOS PRIMEROS DÍAS TRAS LA INFECCIÓN
  real x0,a=0.1, b=media;
  int error;

  int NumeroIteraciones=mn_regula_falsi (f,a,b,x0,TOL,NiterMax);
  cout << "\nRegula Falsi  : x0 = " << x0 << " , N. iteraciones = " <<
  NumeroIteraciones << " , Gamma(x0) = " << Gamma(x0) << "\n";

  /// CALCULO EN LOS ÚLTIMOS DÍAS TRAS LA INFECCIÓN
  a=media, b=20;
  real x1;
  NumeroIteraciones=mn_regula_falsi (f,a,b,x1,TOL,NiterMax);
  cout << "\nRegula Falsi  : x1 = " << x1 << " , N. iteraciones = " <<
  NumeroIteraciones << " , Gamma(x1) = " << Gamma(x1) << "\n";

  /// CALCULO DE DONDE ALCANZA EL MÁXIMO LA DISTRIBUCIÓN GAMMA
  /// USANDO EL MÉTODO DE LA REGULA-FALSI
  a=x0, b=x1;
  real xmax;
  NumeroIteraciones=mn_regula_falsi (Gammap,a,b,xmax,TOL,NiterMax);
  cout << "\nRegula Falsi  : xmax = " << xmax << " , N. iteraciones = " <<
