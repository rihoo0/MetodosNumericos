...
Array1D< real > mn_gauss(
  const Array2D< real > &A0  /** MATRIZ DEL SISTEMA */,
  const Array1D< real > &b0) /** VECTOR DE TERMINOS INDEPENDINENTES */
{
  /** HACER ALUMNO */
  Array2D< real > A=A0.copy();
  Array1D< real > b=b0.copy();
  ...
  {
     real sum=0;
     for(int l=k+1;l<b.dim();l++) sum+= A[k][l]*u[l];
     u[k]=(b[k]-sum)/A[k][k];
  }
