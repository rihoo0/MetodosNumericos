...
/// ------------------------------------------------------------
Array1D< real > mn_remonte (Array2D< real > &B,Array1D< real > &z,Array1D< int > &piv)
{
   int i,j;
   ...
   for (i=N-1;i>=0;i--){
      if (B[piv[i]][i]==0){  /// comprobamos que la diagonal es distinto de cero
         return(Array1D< real >());
      }
      DU[i]=z[piv[i]]; /// inicializamos la solucion
      for  (j=i+1;j<N;j++){ /// aplicamos la formula para calcuLar la solucion
         DU[i]=DU[i] - B[piv[i]][j]*DU[j];
      }
      DU[i]=DU[i]/B[piv[i]][i];
  }
  return(DU);
...
EL MÁXIMO EN VALOR ABSOLUTO DE UNA MATRIZ Y TENIENDO EN CUENTA LA PIVOTACIÓN */
int max_pos(
Array2D< real > &A /** matriz */,
int k /** posición diagonal */,
Array1D< int > &piv /** vector pivotación */){
  real max=fabs(A[piv[k]][k]);
  int kmax=k;
  for(int m=k+1;m<A.dim1();m++){
    if(fabs(A[piv[m]][k])>max){
      max=fabs(A[piv[m]][k]);
      kmax=m;
    }
  }
...
/** FUNCION PARA RESOLVER SISTEMAS POR EL METODO DE GAUSS CON PIVOTACIÓN VIRTUAL
...
Array1D< real > &b_original) /** VECTOR DE TERMINOS INDEPENDINENTES */
{
  /// HACER ALUMNO
    ...
    for(int k = 0; k < N - 1; k++){
        ...
        for(int i = k + 1; i < N; i++){
            int m = A[piv[i]][k] / A[piv[k]][k];
            A[i][k] = 0;
            for(int j = k + 1; j < N; j++){
                A[piv[i]][j] = A[piv[i]][j] - m * A[piv[k]][j];
            }
            b[piv[i]] = b[piv[i]] - m * b[piv[k]];
        }
    }
