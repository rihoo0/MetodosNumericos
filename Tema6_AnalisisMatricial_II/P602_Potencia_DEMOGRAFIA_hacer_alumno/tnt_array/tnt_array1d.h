...
class Array1D
{
  ...
  public:
    ...
    explicit Array1D(const std::string &filename);
    double norma() const;

};

template <class T>
double Array1D<T>::norma() const
{
    double s = 0.0;
    for (int i = 0; i < n_; ++i)
        s += (double) data_[i]*data_[i];
    return std::sqrt(s);
}
...
template <class T>
bool Array1D<T>::write(const std::string &filename) const
{
...
template <class T>
Array1D<T>::Array1D(const std::string &filename)
    : v_(), n_(0), data_(0)
{
...
template <class T>
double Array1D<T>::suma(){
  double s=0.;
  for(int k=0;k<dim();k++) s+=v_[k];
  return s;
}
